...
def test_calculate_position_size(mock_connector):
    ...
    mock_connector.get_leverage_tiers.return_value = [
        {'max_leverage': 10.0}
    ]
...
def test_calculate_hedge_parameters():
    ...
    hedge_size_invalid, hedge_leverage_invalid = risk_manager.calculate_hedge_parameters(
        main_position_size=main_size,
        main_leverage=main_leverage,
...
def test_validate_trade(mock_connector):
    ...
    risk_manager.positions['ETH'] = {'symbol': 'ETH', 'size': 1.0, 'mark_price': 1500.0, 'leverage': 5.0}
