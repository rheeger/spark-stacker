...
class TestRealMarketDataUsage:
    ...
    def setup_method(self):
        ...
        self.risk_manager.calculate_position_size.return_value = (1.0, 5.0)  # (size, leverage)
        self.risk_manager.calculate_hedge_parameters.return_value = (0.2, 2.0)  # (size, leverage)
    ...
    def test_macd_signal_from_real_market_data(self):
        ...
        if len(all_signals) == 0:
            ...
            signal = Signal(
                ...
                params={
                    ...
                    "histogram": last_row["macd_histogram"] if "macd_histogram" in last_row else 0
                }
            )
        else:
        ...
        self._visualize_macd_data(market_data, processed_data, all_signals, "macd_signals_from_real_data")
    ...
    def _generate_test_data(self):
        ...
        for i in range(len(df)):
            df.loc[df.index[i], 'high'] = max(df.loc[df.index[i], 'open'], df.loc[df.index[i], 'close'], df.loc[df.index[i], 'high'])
            df.loc[df.index[i], 'low'] = min(df.loc[df.index[i], 'open'], df.loc[df.index[i], 'close'], df.loc[df.index[i], 'low'])
    ...
    def _visualize_macd_data(self, price_data, macd_data, signals, filename):
        ...
        try:
            ...
            buy_signals = [(t, s) for t, s in signals if s.direction == SignalDirection.BUY]
            sell_signals = [(t, s) for t, s in signals if s.direction == SignalDirection.SELL]

            if buy_signals:
                buy_timestamps = [t for t, _ in buy_signals]
                buy_prices = price_data.loc[buy_timestamps]['close']
                ax1.scatter(buy_timestamps, buy_prices, color='green',
                          label='Buy Signal', marker='^', s=100)

            if sell_signals:
                sell_timestamps = [t for t, _ in sell_signals]
                sell_prices = price_data.loc[sell_timestamps]['close']
                ax1.scatter(sell_timestamps, sell_prices, color='red',
            ...
            if 'macd' in macd_data.columns and 'macd_signal' in macd_data.columns:
                ax2.plot(macd_data.index, macd_data['macd'], color='blue', label='MACD Line')
                ax2.plot(macd_data.index, macd_data['macd_signal'], color='red', label='Signal Line')
                ...
                if 'macd_histogram' in macd_data.columns:
                    ...
                    ax2.bar(macd_data.index, positive_hist, color='green', alpha=0.5, label='Positive Histogram')
                    ax2.bar(macd_data.index, negative_hist, color='red', alpha=0.5, label='Negative Histogram')
