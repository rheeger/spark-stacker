...
def price_data():
    ...
    return pd.DataFrame({
        'open': [100, 105, 110, 115, 120, 115, 110, 105, 100, 95, 90, 85, 80, 85, 90],
        'high': [105, 110, 115, 120, 125, 120, 115, 110, 105, 100, 95, 90, 85, 90, 95],
        'low': [95, 100, 105, 110, 115, 110, 105, 100, 95, 90, 85, 80, 75, 80, 85],
        'close': [105, 110, 115, 120, 115, 110, 105, 100, 95, 90, 85, 80, 85, 90, 95],
        'volume': [1000] * 15
    }, index=pd.date_range(start='2023-01-01', periods=15, freq='D'))

def test_indicator_signal_to_engine(price_data, mock_connector, mock_risk_manager, trading_engine):
    """
    ...
    rsi = RSIIndicator(name="RSI", params={"period": 5, "overbought": 70, "oversold": 30, "signal_period": 1})
...
def test_webhook_to_indicator_to_engine(trading_engine, webhook_server, mock_connector, mock_risk_manager):
    """
    ...
    pytest.skip("This test requires a running Flask server and is meant for local testing only")
