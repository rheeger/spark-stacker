...
class TestGeneticOptimizationWorkflow:
    ...
    def test_genetic_optimization_workflow(self, backtest_engine, sample_data_directory):
        ...
        bollinger = BollingerBandsIndicator(name="Bollinger", params={"period": 20, "num_std_dev": 2.0})
        ...
        for metric_name, metric_value in best_result.metrics.items():
            if metric_name in ['sharpe_ratio', 'total_return', 'win_rate', 'max_drawdown']:
        ...
        os.makedirs(os.path.join(sample_data_directory, 'genetic_results'), exist_ok=True)

        # 9. Save results
        best_result_file = os.path.join(sample_data_directory, 'genetic_results', 'genetic_optimization_result.json')
        best_result.save_to_file(best_result_file)

        validation_result_file = os.path.join(sample_data_directory, 'genetic_results', 'validation_result.json')
        validation_result.save_to_file(validation_result_file)

        # 10. Generate and save equity curve plots
        train_equity_curve_file = os.path.join(sample_data_directory, 'genetic_results', 'train_equity_curve.png')
        best_result.plot_equity_curve(save_path=train_equity_curve_file)

        validation_equity_curve_file = os.path.join(sample_data_directory, 'genetic_results', 'validation_equity_curve.png')
        ...
        print(f"Random params - Sharpe: {random_result.metrics['sharpe_ratio']:.4f}, Return: {random_result.metrics['total_return']*100:.2f}%")
        print(f"Optimized params - Sharpe: {best_result.metrics['sharpe_ratio']:.4f}, Return: {best_result.metrics['total_return']*100:.2f}%")
        ...
        if random_result.metrics['sharpe_ratio'] != 0:
            performance_improvement = (best_result.metrics['sharpe_ratio'] - random_result.metrics['sharpe_ratio']) / abs(random_result.metrics['sharpe_ratio']) * 100
            print(f"Performance improvement: {performance_improvement:.2f}%")
        else:
            print("Cannot calculate percentage improvement as baseline sharpe ratio is zero")
