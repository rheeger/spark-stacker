...
class RSIIndicator(BaseIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(data) < self.period + 1:
            logger.warning(f"Not enough data points for RSI calculation. Need at least {self.period + 1}, got {len(data)}")
        ...
        # Detect crossovers
        df['leaving_oversold'] = (df['rsi'] > self.oversold) & (df['rsi'].shift(1) <= self.oversold)
        df['leaving_overbought'] = (df['rsi'] < self.overbought) & (df['rsi'].shift(1) >= self.overbought)
    ...
    def generate_signal(self, data: pd.DataFrame) -> Optional[Signal]:
        ...
        # Check for sell signal (leaving overbought)
            ...
            prev_rsi = data.iloc[-2]['rsi']
            confidence = min(1.0, (prev_rsi - self.overbought) / (100 - self.overbought) + 0.5)
