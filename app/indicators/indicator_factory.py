...
class IndicatorFactory:
    ...
    @classmethod
    def create_indicator(cls, name: str, indicator_type: str, params: Optional[Dict[str, Any]] = None) -> Optional[BaseIndicator]:
        """
        ...
        if indicator_type not in cls._indicator_registry:
            logger.error(f"Indicator type '{indicator_type}' not supported. Available types: {list(cls._indicator_registry.keys())}")
            return None
        ...
        try:
            ...
            logger.error(f"Failed to create indicator '{name}' of type '{indicator_type}': {e}")
            return None
    
    @classmethod
    def create_indicators_from_config(cls, configs: List[Any]) -> Dict[str, BaseIndicator]:
        """
        ...
        for config in configs:
            ...
            if hasattr(config, 'name') and hasattr(config, 'type'):
                ...
                logger.warning(f"Skipping invalid indicator config type: {type(config)}")
                continue
            
            logger.debug(f"Processing indicator config: name={name}, type={indicator_type}, enabled={enabled}")
            
            if not name or not indicator_type:
                logger.warning(f"Skipping invalid indicator config: missing name or type")
                continue
