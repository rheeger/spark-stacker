...
class ConnectorFactory:
    ...
    def create_connector(cls, 
                        ...
                        market_types: Optional[List[Union[str, MarketType]]] = None) -> Optional[BaseConnector]:
        """
        ...
        logger.info(f"Creating connector for exchange type: {exchange_type} with name: {connector_name}")
        ...
        logger.info(f"Connector will support market types: {[mt.value for mt in processed_market_types]}")
        ...
        try:
            ...
            if connector:
                ...
                try:
                    ...
                    if (not hasattr(connector, 'balance_logger') or 
                        not hasattr(connector, 'markets_logger') or 
                        not hasattr(connector, 'orders_logger') or 
                        connector.balance_logger is None or 
                        connector.markets_logger is None or 
                        connector.orders_logger is None):
                        connector.setup_loggers()
                    else:
                        logger.debug(f"Loggers already set up for {connector.name}, skipping setup")
                except Exception as e:
                    logger.warning(f"Error setting up loggers for {connector.name}: {e}, continuing anyway")
    ...
    @classmethod
    def _process_market_types(cls, exchange_type: str, market_types: Optional[List[Union[str, MarketType]]]) -> List[MarketType]:
        """
