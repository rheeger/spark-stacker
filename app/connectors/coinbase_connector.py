...
class CoinbaseConnector(BaseConnector):
        ...
        try:
            ...
            if order_type == OrderType.MARKET:
                ...
                try:
                    ...
                    try:
                        ...
                        self.orders_logger.info(
                            f"Placing limit order with config: {order_config}"
                        )
            ...
            result = {
                ...
                "status": self._map_order_status(
                    getattr(order_details, "status", "open")
                ),
                "timestamp": getattr(
                    order_details, "created_time", int(time.time() * 1000)
                ),
            }
        ...
        try:
            ...
            if not response:
                self.orders_logger.error(
                    f"Failed to cancel order {order_id}: Empty response"
                )
                return False
    ...
    def get_order_status(self, order_id: str) -> Dict[str, Any]:
        ...
        self.orders_logger.info(
            f"Order status for {order_id}: {result.get('status', 'unknown')}"
        )
        return result
    ...
    def get_order(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(order, "order_configuration"):
                if hasattr(order.order_configuration, "limit_limit_gtc") or hasattr(
                    order.order_configuration, "limit_limit_gtd"
                ):
            ...
            if order_type == OrderType.LIMIT.value and hasattr(
                order.order_configuration, "limit_limit_gtc"
            ):
    ...
    def set_leverage(self, symbol: str, leverage: float) -> Dict[str, Any]:
        ...
        return {"success": False, "message": "not supported for Coinbase spot trading"}
