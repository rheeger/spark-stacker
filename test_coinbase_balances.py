...
def main():
    try:
        ...
        results = {
            ...
            "response_attrs": [attr for attr in dir(accounts_response) if not attr.startswith('_') and not callable(getattr(accounts_response, attr))],
            "accounts": []
        }
        ...
        if hasattr(accounts_response, 'accounts'):
            ...
            non_zero_balances = 0
            for i, account in enumerate(accounts_response.accounts[:20]):  # Show first 20 accounts
                acct_data = {"index": i}
                ...
                if hasattr(account, 'available_balance') and hasattr(account.available_balance, 'value'):
                    ...
                    acct_data["balance_currency"] = getattr(account.available_balance, 'currency', None)
