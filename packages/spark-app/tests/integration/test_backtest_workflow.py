...
class TestBacktestWorkflow:
    ...
    def test_complete_backtest_workflow(self, backtest_engine, sample_data_directory):
        ...
        for name, strategy in strategies.items():
            ...
            result_file = os.path.join(sample_data_directory, 'results', f"{name.lower().replace(' ', '_')}_result.json")
        ...
        for name, result in results.items():
            print(f"{name} - Sharpe Ratio: {result.metrics['sharpe_ratio']:.4f}, Return: {result.metrics['total_return']*100:.2f}%, Max Drawdown: {result.metrics['max_drawdown']*100:.2f}%")
        ...
        if wfa_results:  # There should be at least one window if the date range is sufficient
        ...
        wfa_file = os.path.join(sample_data_directory, 'results', 'walk_forward_analysis.json')
        with open(wfa_file, 'w') as f:
            json.dump(wfa_results, f, indent=2)

        # 7. Create equity curve plot for the best strategy
        best_strategy_name = max(results, key=lambda k: results[k].metrics['sharpe_ratio'])
