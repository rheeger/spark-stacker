...
class TestMACDIndicatorWithRealData:
    ...
    def test_macd_performance_across_assets(self, real_market_data, market_data_params):
        ...
        print(f"{symbol}: BUY: {len(buy_signals)} ({buy_pct:.1f}%), SELL: {len(sell_signals)} ({sell_pct:.1f}%)")
    ...
    @pytest.mark.parametrize('params', [
        {'fast_period': 8, 'slow_period': 21, 'signal_period': 5},  # More responsive
        {'fast_period': 16, 'slow_period': 32, 'signal_period': 12}  # Less responsive
    ])
    def test_macd_with_various_parameters(self, sample_price_data, params):
        ...
        buy_signals = signals[signals["signal"] == SignalDirection.BUY]
        sell_signals = signals[signals["signal"] == SignalDirection.SELL]
        print(f"Fast: {params['fast_period']}, Slow: {params['slow_period']}, Signal: {params['signal_period']}")
    ...
    def _save_chart(self, price_data, signals, filename_prefix):
        ...
        try:
            ...
            ax2.plot(signals.index, signals['macd_signal'], color='red', label='Signal Line')
            ...
            ax2.bar(signals.index, positive_hist, color='green', alpha=0.5, label='Positive Histogram')
            ax2.bar(signals.index, negative_hist, color='red', alpha=0.5, label='Negative Histogram')
