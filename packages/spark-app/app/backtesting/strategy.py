...
def simple_moving_average_crossover_strategy(
    data: pd.DataFrame, simulation: SimulationEngine, params: Dict[str, Any]
) -> None:
    """
    ...
    if fast_above_slow and not prev_fast_above_slow:
        logger.info(
            f"Buy signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')}"
        )
    ...
    # Sell signal: Fast MA crosses below Slow MA
        ...
        logger.info(
            f"Sell signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')}"
        )
...
def bollinger_bands_mean_reversion_strategy(
    ...
    if current_close <= current_lower:
        logger.info(
            f"Buy signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - Price below lower band"
        )
    ...
    # Sell signal: Price touches or crosses above upper band
        ...
        logger.info(
            f"Sell signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - Price above upper band"
        )
...
def rsi_strategy(
    ...
    if current_rsi < oversold and (prev_rsi is None or prev_rsi >= oversold):
        logger.info(
            f"Buy signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - RSI below oversold"
        )
    ...
    # Sell signal: RSI crosses above overbought level
        ...
        logger.info(
            f"Sell signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - RSI above overbought"
        )
...
def macd_strategy(
    ...
    if (
        macd_above_signal
        and prev_macd_above_signal is not None
        and not prev_macd_above_signal
    ):
        logger.info(
            f"Buy signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - MACD crossed above signal"
        )
    ...
    # Sell signal: MACD crosses below signal line
        ...
        not macd_above_signal
        and prev_macd_above_signal is not None
        and prev_macd_above_signal
    ):
        logger.info(
            f"Sell signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - MACD crossed below signal"
        )
...
def multi_indicator_strategy(
    ...
    if buy_signals >= 2:
        logger.info(
            f"Buy signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - {buy_signals} buy signals active"
        )
        ...
        logger.info(
            f"Sell signal detected at {pd.to_datetime(current_candle['timestamp'], unit='ms')} - {sell_signals} sell signals active"
        )
